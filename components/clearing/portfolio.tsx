...
import { ArrowLeft, ArrowRight, ArrowUpRight, BookOpen, Check, Maximize2, Minus, Plus, X } from 'lucide-react';
...
function ProjectCard({ project, onOpen, visited }: { project: PortfolioProject; onOpen: (id: PortfolioId, page?: number) => void; visited: boolean }) {
  ...
  return <button className="portfolio-card" onClick={() => onOpen(project.id, project.start)} onPointerEnter={() => {
    ...
    <div className="portfolio-card-meta"><span>{project.number} / {project.theme}</span><span>{visited ? <><Check size={12} /> Visited</> : `${pages.length} pages`}</span></div>
...
export function PortfolioIndex({ onOpen, onReadAll, visited }: { onOpen: (id: PortfolioId, page?: number) => void; onReadAll: () => void; visited: string[] }) {
  return <section className="portfolio-section" id="portfolio">
    <div className="section-heading"><div><p className="eyebrow">The complete design portfolio · 2023</p><h2>In Seek of Homeland<span className="heading-period">.</span></h2></div><Button variant="outline" className="read-portfolio" onClick={onReadAll}><BookOpen size={16} /> Read all 45 pages <ArrowUpRight size={16} /></Button></div>
    <div className="portfolio-preface"><p>Seven projects on water, memory, ecology, and the places we call home.<br className="desktop-break" /> Follow a curiosity, or wander through the whole collection.</p><span className="scrub-hint">Move across a drawing to leaf through <ArrowRight size={15} /></span></div>
    ...
    <div className="portfolio-end"><span>7 projects + personal drawings · All original pages included</span><Button variant="ghost" className="text-action" onClick={onReadAll}>Open the portfolio from the beginning <ArrowRight size={16} /></Button></div>
  </section>;
}
...
export function PortfolioReader({ entry, page, onPageChange, onProjectChange, onClose }: {
  ...
  return <Dialog open={entry !== null} onOpenChange={open => { if (!open) onClose(); }}>
    <DialogContent className="folio-dialog" showCloseButton={false} onKeyDown={event => {
      ...
      <header className="folio-header"><div><p className="eyebrow">Xinyi Zhang / Design portfolio</p><DialogTitle className="folio-title">{current?.title ?? 'In Seek of Homeland'}</DialogTitle><DialogDescription className="folio-description">{current?.subtitle ?? 'The complete collection · 45 original pages'}</DialogDescription></div>
        <div className="folio-header-actions"><Select value={entry ?? 'landscape'} onValueChange={value => { if (value) onProjectChange(value as PortfolioId | 'landscape'); }}><SelectTrigger className="chapter-picker" aria-label="Choose portfolio chapter"><SelectValue>{current ? `${current.number} / ${current.title}` : 'All 45 pages'}</SelectValue></SelectTrigger><SelectContent className="chapter-options" alignItemWithTrigger={false}><SelectItem value="landscape">All 45 pages</SelectItem>{portfolio.map(p => <SelectItem value={p.id} key={p.id}>{p.number} / {p.title}</SelectItem>)}</SelectContent></Select><Button variant="ghost" size="icon" className="reader-close" aria-label="Close portfolio" onClick={onClose}><X size={21} /></Button></div>
      </header>
      ...
      <div className="folio-credit"><span>{chapter?.title ?? 'In Seek of Homeland'} · Original project credits appear on the introductory page.</span>{current && <button onClick={() => onProjectChange('landscape', page)}>Continue in the complete portfolio <ArrowUpRight size={13} /></button>}</div>
    </DialogContent>
  </Dialog>;
}
