...
场景铺满探索区域。移动鼠标可以环顾，拖动带有惯性，点击地面向该位置靠近；项目入口在鼠标接近时显现。右上角支持放大、缩小、全景和沉浸视图，键盘方向键可以平移，Home 回到全景。触屏可横向滑动，放大后可自由拖动和双指缩放。滚动或点击 Walk on 继续沿路径浏览；始终可以回到 A Clearing。系统选择减少动态效果时，关闭视差和惯性并直接切换视角。
